...
import { Alert, Animated, Dimensions, Image, Keyboard, Modal, Platform, Pressable, StyleSheet, View } from 'react-native';
...
export default function AIConfirmation2Screen() {
  ...
  useEffect(() => {
    if (showModal) {
      const keyboardDidShowListener = Keyboard.addListener('keyboardDidShow', () => {
        setIsModalKeyboardVisible(true);
      });
      const keyboardDidHideListener = Keyboard.addListener('keyboardDidHide', () => {
        setIsModalKeyboardVisible(false);
      });
  ...
  const handleModalStartRecording = async () => {
    try {
      await speechService.startRecording();
      setIsModalRecording(true);
      setIsModalKeyboardVisible(false);
    } catch (error) {
      console.error('Error starting recording in modal:', error);
      setIsModalRecording(true);
    }
  };

  const handleModalVoicePress = async () => {
    if (isModalKeyboardVisible) {
      // When keyboard is visible, send the text
      handleModalSend();
    } else {
      // Start recording
      await handleModalStartRecording();
    }
  };
  ...
  const handleModalSend = async () => {
    ...
    if (isModalRecording) {
      try {
        ...
        console.error('Error transcribing in modal:', error);
      }
      setIsModalRecording(false);
    }
          ...
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={styles.modalInputContainer}>
              <TextAndVoiceInput
                ...
                onVoicePress={handleModalVoicePress}
                onStartRecording={handleModalStartRecording}
                onSend={handleModalSend}
                isKeyboardVisible={isModalKeyboardVisible}
                autoFocus={false}
                multiline={false}
              />
            </View>
