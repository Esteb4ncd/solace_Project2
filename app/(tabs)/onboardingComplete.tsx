...
export default function OnboardingCompleteScreen() {
	...
	const { setRecommendedExercises, updateDailyTasks } =
		useExerciseContext();
	...
	useEffect(() => {
		...
		if (selectedWorkTasks && selectedPainAreas) {
			try {
				const workTasks = JSON.parse(
					selectedWorkTasks as string
				);
				const painAreas = JSON.parse(
					selectedPainAreas as string
				);
				...
				if (
					...
					if (recommendedExercises.length > 0) {
						...
						console.log(
							"✅ Generated recommendations from manual selection:",
							dailyTasks
						);
					}
				}
				...
				console.error(
					"Error generating recommendations from manual selection:",
					error
				);
			}
		}
		...
		<View style={styles.container}>
			...
			<View style={styles.content}>
				...
				<Text style={styles.subtitle}>
					We've set your stretch plan based on
					your answer.
				</Text>
			</View>
